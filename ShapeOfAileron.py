...
def shapeOfAileron(x_coords, displ_na, d_theta, plot_aileron=False, plot_deflections_theta_0=False, plot_deflections=False):
	...
	# Seems trivial but very important, make a copy of the input! Don't override it
	displ_na_global = displ_na.copy()
	...
	for i in range(len(displ_na[0])):
		...
		displ_na_global[0][i] = r*np.sin(angle)
		displ_na_global[1][i] = r*np.cos(angle)
	...
	le_y_coords = displ_na_global[0] + np.sin(section_thetas)*((h_a/2.))
	...
	le_z_coords = displ_na_global[1] + np.cos(section_thetas)*((h_a/2.))
	...
	# in the y direction.
	te_y_coords = displ_na_global[0] + np.sin(section_thetas+np.pi)*(c_a - (h_a/2.))
	...
	te_z_coords = displ_na_global[1] + np.cos(section_thetas+np.pi)*(c_a - (h_a/2.))
	...
	if plot_aileron:
		...
		# pass the coordinates in the order x, z, y as opposed to x, y, z.
		# The axis will be labelled to match the axis system we used.
		ax.plot(x_coords, displ_na_global[1], displ_na_global[0], color='blue', label='NA')			# This bit will plot the NA.
	...
	if plot_deflections_theta_0:
		...
		plt.plot(x_coords, displ_na_global[0], label='NA y deflection', marker='+')
		plt.plot(x_coords, displ_na_global[1], label='NA z deflection', marker='x')
