...
def plotLETE(U_LEs_FEM, U_TEs_FEM, LE_xlocs, TE_xlocs, correction_LE, correction_TE, version = '', span_disc = [], U_LE_Model=[], U_TE_Model=[]):
    ...
    if len(span_disc):
        ...
        plt.clf()
        axs = plt.axes()
        plt.title('Num. and shifted Val. Model -- LE Deflection Magnitude -- Max (Num.): %.2f mm' % np.max( np.sqrt(U_LE_Model[0,:]**2 + U_LE_Model[1,:]**2) ) , fontsize=20)
        plt.ylabel('Displacement magnitude [mm]', fontsize=18)
        plt.xlabel('x location (spanwise) [mm]', fontsize=18)
        ...
        plt.clf()
        axs = plt.axes()
        plt.title('Num. and shifted Val. Model -- TE Deflection Magnitude -- Max (Num.): %.2f mm' % np.max( np.sqrt(U_TE_Model[0,:]**2 + U_TE_Model[1,:]**2) ), fontsize =20)
        plt.ylabel('Displacement magnitude [mm]', fontsize=18)
        plt.xlabel('x location (spanwise) [mm]', fontsize=18)
    ...
    plt.ylabel('y-displacement [mm]',fontsize=18)
    ...
    plt.subplot(212)
    plt.xlabel('x location (spanwise) [mm]',fontsize=18)
    plt.ylabel('z-displacement [mm]',fontsize=18)
    ...
    axs = plt.axes()
    plt.subplot(211)
    plt.title('Validation Data -- Global TE Deflection -- Maximum: %.2f mm' % np.max(U_TEs_FEM[1,:,1]),fontsize=20)
    plt.ylabel('y-displacement [mm]',fontsize=18)
